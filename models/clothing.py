...
def get_clothing_category(category):
    clothing = session.query(Clothing).filter_by(clothing_category=category).all()
    return jsonify(clothing=[l.serialize for l in clothing])

def makeANewClothing(clothing_name, clothing_category, clothing_price, kibbonbon_lvl, clothing_location, clothing_image):
    add_clothing = Clothing(clothing_name=clothing_name, clothing_category=clothing_category, clothing_price = clothing_price, kibbonbon_lvl = kibbonbon_lvl, clothing_location = clothing_location, clothing_image = clothing_image)
    session.add(add_clothing)
    session.commit()
    return jsonify(Clothing=add_clothing.serialize)

def updateClothing(id, clothing_name, clothing_category, clothing_price, kibbonbon_lvl, clothing_location, clothing_image):
    ...
    if not clothing_category:
        updatedClothing.category = clothing_category

...
from models.ooblets import get_ooblets, get_ooblet, makeANewOoblet, updateOoblet, deleteAOoblet, get_ooblet_location
from models.characters import get_characters, get_character, makeANewCharacter, updateCharacter, deleteACharacter
from models.tools import get_tools, get_tool, makeANewTool, updateTool, deleteATool
from models.crops import get_crops, get_crop, makeANewCrop, updateCrop, deleteACrop
from models.housing import get_housing, get_house, makeANewHouse, updateHouse, deleteAHouse
from models.clothing import get_clothings, get_clothing, makeANewClothing, updateClothing, deleteAClothing
from models.badges import get_badges, get_badge, makeANewBadge, updateBadge , deleteABadge
...
def oobletsFunctionId(id):
    if request.method == 'GET':
        ...
        signature_move_lvl_3_effect = request.args.get('signature_move_lvl_3_effect', '')
        ooblet_image = request.args.get('ooblet_image', '')
        return updateOoblet(id, name,  description, location, battle_requirements,signature_move_lvl_1_name,signature_move_lvl_1_beat_requirement,signature_move_lvl_1_effect,signature_move_lvl_2_name,signature_move_lvl_2_beat_requirement,signature_move_lvl_2_effect,signature_move_lvl_3_name,signature_move_lvl_3_beat_requirement,signature_move_lvl_3_effect,ooblet_image)

    elif request.method == 'DELETE':
        return deleteAOoblet(id)

@app.route('/oobletsApi/<string:location>', methods=['GET', 'PUT', 'DELETE'])
def oobletsFunctionLocation(location):
    if request.method == 'GET':
        return get_ooblet_location(location)
...
def charactersFunction():
    if request.method == 'GET':
        ...
        return makeANewCharacter(character_name, character_location, character_image, gender, personality, occupation)
...
def charactersFunctionId(id):
    if request.method == 'GET':
        ...
        character_image = request.args.get('character_image', '')
        occupation = request.args.get('occupation', '')
        return updateCharacter(id, character_name, character_location, character_image, gender, personality, occupation)
...
def clothingFunction():
    if request.method == 'GET':
        ...
        return makeANewClothing(clothing_name, category, clothing_price, kibbonbon_lvl, clothing_location, clothing_image)
...
def clothingFunctionId(id):
    if request.method == 'GET':
        ...
        clothing_location = request.args.get('clothing_location', '')
        clothing_image = request.args.get('clothing_image', '')
        return updateClothing(id, clothing_name, category, clothing_price, kibbonbon_lvl, clothing_location, clothing_image)
